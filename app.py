            ...
            inputs = [
                {
                    ...
                    "content": [
                        {
                            "type": "text", 
                            "text": "IMPORTANT: First check if this page contains a visible barcode/QR code image.\n\nBarcode Image Present: [YES or NO]\n\nIf NO barcode image is present on this page, respond ONLY with 'Barcode Image Present: NO' and do NOT extract any data.\n\nIf YES barcode image is present, continue with extraction.\n\n---\n\nAlso check if this page contains 'DROP SHEET'. If it does:\nDROP SHEET\n\nIf this page is a DROP SHEET without a barcode image, do NOT extract any data.\nIf this page is a DROP SHEET with a barcode image, continue to extract data.\n\n---\n\nIf this is NOT a DROP SHEET page and HAS a barcode image, extract ONLY PRINTED/TYPED text from this document. Skip all handwritten text completely.\n\n🚨 CRITICAL EXTRACTION RULES 🚨\n\n1. BLANK FIELD RULE: If a field is NOT present in the document, leave it COMPLETELY BLANK. DO NOT write placeholder text like '[NOT FOUND]', 'N/A', '(not printed)', '[NOT PROVIDED]', or any similar text. Just leave it empty.\n\n2. DO NOT COPY FIELDS: Each field must be extracted independently from its own location in the document. NEVER copy a value from one field to another field. If you cannot find a specific field, leave it blank - do NOT use data from a different field.\n\n---\n\n*** CRITICAL: SUB/MEMBER NO. vs GROUP NUMBER - READ CAREFULLY ***\n\nThese are TWO COMPLETELY DIFFERENT fields that are NEVER the same:\n\n📋 Sub/Member No. (PERSONAL ID):\n- What it is: The individual patient's/subscriber's personal identification number\n- Where to find it: Look for these EXACT labels in the document:\n  * \"Member ID\" or \"Member #\"\n  * \"Subscriber ID\" or \"Subscriber #\" or \"Sub #\"\n  * \"Member No.\" or \"Mem No.\"\n  * \"ID Number\" (in insurance section)\n- Characteristics: Usually 8-15 characters, alphanumeric, complex format\n- Examples: 5M62HF1DR98, S00834933001, W1234567890, YXT123456789\n- Location: Usually found near patient/subscriber information\n\n🏢 Group Number (EMPLOYER/ORGANIZATION ID):\n- What it is: The employer's or organization's group identification code\n- Where to find it: Look for these EXACT labels in the document:\n  * \"Group Number\" or \"Group #\" or \"Grp #\"\n  * \"Group ID\" or \"Group Code\"\n  * \"Group Plan\"\n- Characteristics: Usually 3-8 characters, simpler format, may be just letters or numbers\n- Examples: ABC123, MCRMA, G12345, 00123, COMP01\n- Location: Usually found near insurance/employer information, often below or separate from Member ID\n\n⚠️ VALIDATION RULES:\n- If you find the same value for both fields, YOU MADE A MISTAKE - go back and check the document labels carefully\n- If a document only shows one of these fields (e.g., only Member ID is printed), extract only that field and leave the other BLANK\n- DO NOT assume one field from another - extract only what is explicitly labeled\n- If you cannot find a field with its proper label, leave it BLANK - do NOT guess or use data from a different field\n\n---\n\nExtract the following information:\n1. Appointment Date (Appt Date) - Extract the appointment date as shown\n2. Name of Patient - Copy EXACTLY as it appears, do NOT rearrange or reformat\n3. Patient Birthdate (DOB) - Extract the date of birth\n4. Facility Information - Include BOTH facility name AND full address together\n5. Primary Insurance Company - Extract the insurance company name\n6. Sub/Member No. - Extract ONLY from fields labeled as Member ID, Subscriber ID, or Member Number (leave blank if not found)\n7. Group Number - Extract ONLY from fields labeled as Group Number, Group ID, or Group # (leave blank if not found)\n8. Diagnosis Codes - Extract all diagnosis/ICD codes\n\nIMPORTANT FOR PATIENT NAME:\n- Copy the name EXACTLY as it appears in the PDF\n- Do NOT rearrange, reformat, or change the order\n- If it says 'DAVID FREITAS W', write 'DAVID FREITAS W' - do NOT change it to 'FREITAS, DAVID'\n- Extract it as-is without any modifications\n\nIMPORTANT FOR DIAGNOSIS CODES:\n- Identify all diagnosis codes in the text (e.g., E11.9, I50.9, I10, J44.9, etc.)\n- Treat the code itself as the unique identifier\n- Ignore descriptions, capitalization differences, punctuation, and line breaks\n- If the same code appears multiple times (even with different descriptions), include it only once\n- If different codes appear, list all unique codes\n- Output only the final list of unique codes, separated by commas with no extra text\n- Do not include explanations or descriptions, only codes\n\nReturn the data in this EXACT format:\nBarcode Image Present: [YES or NO]\nAppt Date: [date or leave blank if not found]\nName of Patient: [name or leave blank if not found]\nPatient Birthdate: [DOB in any date format found or leave blank if not found]\nFacility Information: [facility name and address or leave blank if not found]\nPrimary: [Insurance Company name or leave blank if not found]\nSub/Member No.: [Member ID ONLY - leave blank if label not found]\nGroup Number: [Group Number ONLY - leave blank if label not found]\nDiagnosis Codes: [code1, code2, code3 or leave blank if not found]\n\n🔍 FINAL CHECKLIST BEFORE RESPONDING:\n✓ Did you leave fields blank if not found (no placeholder text)?\n✓ Are Sub/Member No. and Group Number DIFFERENT values?\n✓ Did you extract each field from its correct labeled location?\n✓ Did you copy patient name exactly as shown?\n✓ For Diagnosis Codes, did you return only comma-separated codes with no descriptions?"
                        },
                        {
                            "type": "image_url",
